...
/**
 * 号码IMSI绑定关系分表算法
 * 按号码前3位进行分表，支持前缀匹配查询优化
 * 与号码资源使用相同的分表策略
 */
@Slf4j
@Component
public class NumberImsiBindingShardingAlgorithm implements PreciseShardingAlgorithm<String>, RangeShardingAlgorithm<String> {
    ...
    public String doSharding(Collection<String> availableTargetNames, PreciseShardingValue<String> shardingValue) {
        ...
        // 构造目标表名
        String targetTable = "number_imsi_binding_" + prefix;
    ...
    public Collection<String> doSharding(Collection<String> availableTargetNames, RangeShardingValue<String> shardingValue) {
        ...
        if (prefix != null) {
            ...
            if (prefix.length() == defaultPrefixLen) {
                ...
                for (String availableTable : availableTargetNames) {
                    if (availableTable.startsWith("number_imsi_binding_" + prefix)) {
                        result.add(availableTable);
                    }
                }
                ...
                String targetTable = "number_imsi_binding_" + tablePrefix;
